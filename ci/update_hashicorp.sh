...
main() {
    ...
    # Special case for nomad's podman driver
    projects=(
        'nomad'
        'nomad-fedora'
    )
    ...
    for p in "${projects[@]}"; do
        ...
        current_version="$(eval "$(grep 'DRIVER_VERSION' ${p}/Containerfile | head -1 | cut -f2 -d\  )"; echo ${DRIVER_VERSION})"
        ...
        sed -i "0,/DRIVER_VERSION=.*$/{s//DRIVER_VERSION=\"${version}\"/}" "${p}/Containerfile"
        sed -i "0,/DRIVER_SHA256=.*$/{s//DRIVER_SHA256=\"${new_sha}\"/}" "${p}/Containerfile"

        git add "${p}/Containerfile"
        git commit --message "${p}: Update ${name} to ${version}"
